...
- **All actions must be based on their corresponding Viem actions** from `viem/tempo/actions`
 ...
 *   chains: [tempo.extend({ feeToken: '0x20c0000000000000000000000000000000000001' })],
 *   transports: {
 *     [tempo.id]: http(),
 *   },
 * })
...
**Important**: Wagmi action tests should follow the same test flows as the corresponding Viem action tests in `viem`. This includes:
...
**Important**: Wagmi hook tests should follow the same test flows as the corresponding Viem action tests in `viem/tempo/actions`. This includes:
- Setting up the same initial state (creating tokens, granting roles, minting tokens, etc.)
- Testing the same behaviors and edge cases
- Using the same test data and assertions where applicable
