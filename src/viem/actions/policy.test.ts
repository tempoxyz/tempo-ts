...
describe.skipIf(!!process.env.CI)('create', () => {
  test('default', async () => {
    ...
    const { receipt, ...result } = await actions.policy.create(client, {
      type: 'whitelist',
    })
  ...
  test.skip('behavior: with initial addresses', async () => {
    ...
    const { policyId } = await actions.policy.create(client, {
      type: 'whitelist',
...
describe.skipIf(!!process.env.CI)('setAdmin', () => {
  test('default', async () => {
    ...
    const { receipt: setAdminReceipt, ...setAdminResult } =
      await actions.policy.setAdmin(client, {
        policyId,
...
describe.skipIf(!!process.env.CI)('modifyWhitelist', () => {
  test('default', async () => {
    ...
    const { receipt: addReceipt, ...addResult } =
      await actions.policy.modifyWhitelist(client, {
        policyId,
...
describe.skipIf(!!process.env.CI)('modifyBlacklist', () => {
  test('default', async () => {
    ...
    const { receipt: addBlacklistReceipt, ...addBlacklistResult } =
      await actions.policy.modifyBlacklist(client, {
        policyId,
        address: account2.address,
        restricted: true,
      })
